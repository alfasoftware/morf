...
public class TestMergeStatementDeepCopyContract extends AbstractShallowAndDeepCopyableTest<MergeStatement> {
  ...
  public static List<Object[]> data() {
    return Arrays.asList(
      testCase(() -> merge().into(TABLE_1).from(SELECT_1).tableUniqueKey(literal(1))),
      testCase(() -> merge().into(TABLE_1).from(SELECT_1).tableUniqueKey(literal(2))),
      testCase(() -> merge().into(TABLE_1).from(SELECT_1).tableUniqueKey(literal(1), literal(1))),
      testCase(() -> merge().into(TABLE_1).from(SELECT_2).tableUniqueKey(literal(1))),
      testCase(() -> merge().into(TABLE_2).from(SELECT_1).tableUniqueKey(literal(1)))
    );
  }
}