...
public class TestSelectStatementDeepCopyContract extends AbstractShallowAndDeepCopyableTest<SelectStatement> {
  ...
  public static List<Object[]> data() {
    return Arrays.asList(
      ...
      testCase("select().from(\"A\").innerJoin(TABLE_1)", () -> select().from("A").innerJoin(TABLE_1)),
      testCase("select().from(\"A\").innerJoin(TABLE_2)", () -> select().from("A").innerJoin(TABLE_2)),
      testCase("select().from(\"A\").innerJoin(TABLE_1).innerJoin(TABLE_1).innerJoin(TABLE_1)", () -> select().from("A").innerJoin(TABLE_1).innerJoin(TABLE_1).innerJoin(TABLE_1)),
      testCase("select().from(\"A\").innerJoin(TABLE_1).innerJoin(TABLE_2)", () -> select().from("A").innerJoin(TABLE_1).innerJoin(TABLE_2)),
