...
public class TestSelectFirstStatementDeepCopyContractUsingBuilder extends AbstractShallowAndDeepCopyableTest<SelectFirstStatement> {
  ...
  public static List<Object[]> data() {
    return Arrays.asList(
      ...
      testCaseWithBuilder("selectFirst(literal(1)).from(\"A\").innerJoin(TABLE_1)", selectFirst(literal(1)).from("A").crossJoin(TABLE_1)),
      testCaseWithBuilder("selectFirst(literal(1)).from(\"A\").innerJoin(TABLE_2)", selectFirst(literal(1)).from("A").crossJoin(TABLE_2)),
      testCaseWithBuilder("selectFirst(literal(1)).from(\"A\").innerJoin(TABLE_1).innerJoin(TABLE_2)", selectFirst(literal(1)).from("A").crossJoin(TABLE_1).crossJoin(TABLE_2)),
