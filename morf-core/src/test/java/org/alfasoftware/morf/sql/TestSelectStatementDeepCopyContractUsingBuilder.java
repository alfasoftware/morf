...
public class TestSelectStatementDeepCopyContractUsingBuilder extends AbstractShallowAndDeepCopyableTest<SelectStatement> {
  ...
  public static List<Object[]> data() {
    return Arrays.asList(
      ...
      testCaseWithBuilder("select().from(\"A\").innerJoin(TABLE_1)", select().from("A").crossJoin(TABLE_1)),
      testCaseWithBuilder("select().from(\"A\").innerJoin(TABLE_2)", select().from("A").crossJoin(TABLE_2)),
      testCaseWithBuilder("select().from(\"A\").innerJoin(TABLE_1).innerJoin(TABLE_1).innerJoin(TABLE_1)", select().from("A").crossJoin(TABLE_1).crossJoin(TABLE_1).crossJoin(TABLE_1)),
      testCaseWithBuilder("select().from(\"A\").innerJoin(TABLE_1).innerJoin(TABLE_2)", select().from("A").crossJoin(TABLE_1).crossJoin(TABLE_2)),
