...
public class TestGraphBasedUpgradeBuilder {
  ...
  /**
   * Verify that {@code CreateDeferredIndexOperationTables} (exclusive, sequence 1)
   * acts as a barrier before any step that modifies unrelated tables, ensuring
   * the deferred index infrastructure tables exist before INSERT statements
   * generated by {@code addIndexDeferred()} are executed.
   */
  @Test
  public void testCreateDeferredIndexTablesRunsBeforeOtherSteps() {
    // CreateDeferredIndexOperationTables is @ExclusiveExecution @Sequence(1)
    // DeferredUser modifies an unrelated table "Product" at sequence 100
    UpgradeStep createTablesStep = new org.alfasoftware.morf.upgrade.upgrade.CreateDeferredIndexOperationTables();
    ...
    upgradeSteps.addAll(Lists.newArrayList(createTablesStep, deferredUserStep));

    GraphBasedUpgrade upgrade = builder.prepareGraphBasedUpgrade(initialisationSql);

    // The exclusive step must be a parent of the deferred user step
    checkParentChild(upgrade, createTablesStep, deferredUserStep);
  }


  /**
   * Verify that two steps using {@code addIndexDeferred()} on different tables
   * can run in parallel — the exclusive barrier only applies to
   * {@code CreateDeferredIndexOperationTables}, not between deferred index users.
   */
  @Test
  public void testDeferredIndexUsersRunInParallel() {
    ...
    when(upgradeTableResolution.getModifiedTables(DeferredUser2.class.getName()))
        .thenReturn(Sets.newHashSet("Customer"));

    upgradeSteps.addAll(Lists.newArrayList(createTablesStep, deferredUser1, deferredUser2));

    GraphBasedUpgrade upgrade = builder.prepareGraphBasedUpgrade(initialisationSql);

    // Both deferred users depend on the exclusive create step
    checkParentChild(upgrade, createTablesStep, deferredUser1);
    checkParentChild(upgrade, createTablesStep, deferredUser2);

    // But they do NOT depend on each other — they can run in parallel
    checkNotParentChild(upgrade, deferredUser1, deferredUser2);
    checkNotParentChild(upgrade, deferredUser2, deferredUser1);
  }


  /**
   * Test step simulating a user of addIndexDeferred() on table Product.
   */
  @Sequence(100L)
  static class DeferredUser extends U1 {}

  /**
   * Test step simulating a user of addIndexDeferred() on table Customer.
   */
  @Sequence(101L)
