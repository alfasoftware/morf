...
public class TestGraphBasedUpgradeBuilder {
  ...
  /**
   * Verify that {@code CreateDeferredIndexes} (exclusive, sequence 1)
   ...
   * generated by {@code addIndex (deferred)()} are executed.
   */
  @Test
  public void testCreateDeferredIndexTablesRunsBeforeOtherSteps() {
    // CreateDeferredIndexes is @ExclusiveExecution @Sequence(1)
    // DeferredUser modifies an unrelated table "Product" at sequence 100
    UpgradeStep createTablesStep = new CreateDeferredIndexes();
    UpgradeStep deferredUserStep = new DeferredUser();

    when(upgradeTableResolution.getModifiedTables(
        CreateDeferredIndexes.class.getName()))
        .thenReturn(Sets.newHashSet("DeferredIndexes"));
  ...
  /**
   * Verify that two steps using {@code addIndex (deferred)()} on different tables
   * can run in parallel — the exclusive barrier only applies to
   * {@code CreateDeferredIndexes}, not between deferred index users.
   */
  @Test
