...
/**
 * Unit tests for {@link DeferredIndexBuilder} — one test per branch
 * of the reconciliation algorithm.
 *
 * @author Copyright (c) Alfa Financial Software Limited. 2026
 */
public class TestDeferredIndexBuilder {
  ...
  private DeferredIndex snapshot;
  ...
  public void setUp() throws SQLException {
    ...
    builder = new DeferredIndexBuilder(connectionResources, dao);
    snapshot = rowWith(DeferredIndexStatus.PENDING, 0);
  }
  ...
  public void testInvalidHappyPathPostgresLockTimeout() throws SQLException {
    ...
    when(dao.findByTableAndIndex(TABLE, INDEX)).thenReturn(Optional.of(rowWith(DeferredIndexStatus.IN_PROGRESS, 0)));
    when(dialect.isIndexValid(connection, TABLE, INDEX)).thenReturn(Optional.of(Boolean.FALSE));
    when(dialect.setLockTimeoutSql(eq(DeferredIndexBuilder.LOCK_TIMEOUT))).thenReturn(Optional.of(LOCK_TIMEOUT_SQL));
  ...
  public void testInvalidDropFailsMarksFailedWithPrefixAndDoesNotCreate() throws SQLException {
    ...
    when(dao.findByTableAndIndex(TABLE, INDEX)).thenReturn(Optional.of(rowWith(DeferredIndexStatus.FAILED, 7)));
    when(dialect.isIndexValid(connection, TABLE, INDEX)).thenReturn(Optional.of(Boolean.FALSE));
  ...
  private static DeferredIndex rowWith(DeferredIndexStatus status, int attempts) {
