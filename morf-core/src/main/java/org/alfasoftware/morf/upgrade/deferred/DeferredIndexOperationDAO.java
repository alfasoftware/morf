 ...
 */
class DeferredIndexOperationDAO {
   ...
   */
  @Inject
  DeferredIndexOperationDAO(SqlScriptExecutorProvider sqlScriptExecutorProvider, SqlDialect sqlDialect) {
   ...
   */
  DeferredIndexOperationDAO(ConnectionResources connectionResources) {
  ...
  void insertOperation(DeferredIndexOperation op) {
    ...
    for (int seq = 0; seq < columnNames.size(); seq++) {
      statements.addAll(sqlDialect.convertStatementToSQL(
        insert().into(tableRef(COL_TABLE))
          .values(
            literal(op.getOperationId()).as("operationId"),
