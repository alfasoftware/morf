...
public class TableDataHomology {
  ...
  private String keyColumnsIds(Record record, List<Column> primaryKeys) {
    ...
    for (Column keyCol : primaryKeys) {
      builder.append(keyCol.getName() + "[" + record.getString(keyCol.getName()) + "]");
    }
  ...
  private String keyColumnsIds(Record record1, Record record2, List<Column> primaryKeys) {
    StringBuilder builder = new StringBuilder();
    for (Column keyCol : primaryKeys) {
      builder.append(keyCol.getName() + "=[" + record1.getString(keyCol.getName()) + "][" + record2.getString(keyCol.getName()) + "]");
    }
    return builder.toString();
  }
  ...
  private Comparable<?> convertToComparableType(Column column, Record record) {
    try {
      ...
      throw new IllegalArgumentException("Failed fetching comparable value for [" + column + "] from record " + record); // NOPMD Deliberately squashing the exception - we know what it means and we prefer ours.
    }
  }
}
