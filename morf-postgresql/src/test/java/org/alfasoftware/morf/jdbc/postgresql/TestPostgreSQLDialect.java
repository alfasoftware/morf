...
public class TestPostgreSQLDialect extends AbstractSqlDialectTest {
  ...
  protected List<String> expectedAutoGenerateIdStatement() {
    return Arrays.asList(
      ...
      "INSERT INTO idvalues (name, nextvalue) VALUES('Test', (SELECT COALESCE(MAX(id) + 1, 1) AS CurrentValue FROM testschema.Test))",
      "INSERT INTO testschema.Test (version, stringField, id) SELECT version, stringField, (SELECT COALESCE(nextvalue, 0) FROM idvalues WHERE (name = 'Test')) + Other.id FROM testschema.Other"
    );
  }
  ...
  protected List<String> expectedInsertWithIdAndVersion() {
    return Arrays.asList(
      "DELETE FROM idvalues where name = 'Test'",
      "INSERT INTO idvalues (name, nextvalue) VALUES('Test', (SELECT COALESCE(MAX(id) + 1, 1) AS CurrentValue FROM testschema.Test))",
      "INSERT INTO testschema.Test (stringField, id, version) SELECT stringField, (SELECT COALESCE(nextvalue, 0) FROM idvalues WHERE (name = 'Test')) + Other.id, 0 AS version FROM testschema.Other"
    );
  }
  ...
  protected List<String> expectedSpecifiedValueInsert() {
    return Arrays.asList(
      "DELETE FROM idvalues where name = 'Test'",
      "INSERT INTO idvalues (name, nextvalue) VALUES('Test', (SELECT COALESCE(MAX(id) + 1, 1) AS CurrentValue FROM testschema.Test))",
      "INSERT INTO testschema.Test (stringField, intField, floatField, dateField, booleanField, charField, id, version, blobField, bigIntegerField, clobField) VALUES ('Escap''d', 7, 11.25, 20100405, TRUE, 'X', (SELECT COALESCE(nextvalue, 1) FROM idvalues WHERE (name = 'Test')), 0, null, 12345, null)"
    );
  }
  ...
  protected List<String> expectedSpecifiedValueInsertWithTableInDifferentSchema() {
    return Arrays.asList(
      ...
      "INSERT INTO MYSCHEMA.Test (stringField, intField, floatField, dateField, booleanField, charField, id, version, blobField, bigIntegerField, clobField) VALUES ('Escap''d', 7, 11.25, 20100405, TRUE, 'X', (SELECT COALESCE(nextvalue, 1) FROM idvalues WHERE (name = 'Test')), 0, null, 12345, null)"
    );
  }
